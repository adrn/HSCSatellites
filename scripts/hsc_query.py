...
class HSCQuery(object):
    ...
    def __init__(self, release_version, preview=False,
                 username=None, password=None,
                 user_env='HSC_SSP_CAS_USER', password_env='HSC_SSP_CAS_PASSWORD',
                 nomail=True, skip_syntax_check=True, delete_job_on_complete=True):
    ...
    def run(self, sql_text, output_file=None, output_format='fits', overwrite=False):
        if output_file is not None and path.exists(output_file) and not overwrite:
            raise IOError('File already exists ({}) -- use "overwrite" to overwrite.'
                          .format(output_file))

        self._current_job = self.submit_job(sql_text, output_format=output_format)
        ...
        for i in range(max_time // interval+1):
            ...
            if self._current_job['status'] == 'error':
                raise HSCQueryError('Query error: ' + self._current_job['error'])
        ...
        self.download_job_results(self._current_job['id'], output_file=output_file)
...
def main(**kwargs):
    query = HSCQuery(kwargs['release_version'],
                     user_env=kwargs['user_env'], password_env=kwargs['password_env'],
                     nomail=kwargs['nomail'], skip_syntax_check=kwargs['skip_syntax_check'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--user-env', default='HSC_SSP_CAS_USER',
                        help='The name of the environment variable that contains '
                             'your STARS account username.')
    parser.add_argument('--password-env', default='HSC_SSP_CAS_PASSWORD',
                        help='The name of the environment variable that contains '
                             'your STARS account password.')

    parser.add_argument('--release-version', '-r', choices='dr1 dr_early'.split(),
    ...
    parser.add_argument('--output', '-o', dest='output_file', default=None, type=str,
                        help='The output name of the output file (with or without '
                             'an extension).')

    # TODO: not supported
    # parser.add_argument('--preview', '-p', action='store_true',
    #                     help='Quick mode (short timeout) useful for trying queries.')

    parser.add_argument('--skip-syntax-check', '-S', action='store_true', default=True,
                        help='Skip syntax checking the query.')
    parser.add_argument('--delete-job', '-d', action='store_true', default=True,
                        help='Delete the job after runnng.')
